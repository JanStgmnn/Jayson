...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName("tournament")
		.setDescription("Used by admins to create a tournament")
	...
	async execute(interaction) {
		...
		if (!data) {
			...
			data.leaderboard = new Map();
			data.currgame = 0;
			tournamentowner = interaction.user.id;
			await data.save();
		}

		if (data.running) {
			return interaction.reply({
				content: "There is already a tournament running!",
				ephemeral: true,
			});
		}
		...
		data.tournamentowner = interaction.user.id;
